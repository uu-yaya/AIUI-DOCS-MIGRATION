---
title: 交互API
source_url: https://aiui-doc.xf-yun.com/project-1/doc-584/
---
...
[**- 3、API 介绍>>>点击跳转**](#API 介绍)
    [**- 4、API 接入说明>>>点击跳转**](#API 接入说明)
...
当请求的输入或输出触发审核拦截时，AIUI会赋予错误码返回（详见[错误码列表](https://aiui-doc.xf-yun.com/project-1/doc-42/ "错误码列表")下的10014和10019说明）
...
具体鉴权参数构建见[鉴权文档](https://aiui-doc.xf-yun.com/project-1/doc-405/ "大模型API服务鉴权")说明。
...
| interact\_mode | string | 是 | 对话交互协议，音频交互时可选：continuous 、oneshot ；文本交互时仅支持：oneshot | oneshot |
...
| iat.compress | string | 是 | 识别结果类型 | 目前仅支持raw |
| iat.format | string | 是 | 识别结果格式 | 目前仅支持json |
| iat.encoding | string | 是 | 识别结果编码格式 | 目前仅支持utf8 |
...
| nlp.compress | string | 是 | 语义结果类型 | 目前仅支持raw |
| nlp.format | string | 是 | 语义结果格式 | 目前仅支持json |
| nlp.encoding | string | 是 | 语义结果编码格式 | 目前仅支持utf8 |
...
| tts.bit\_depth | int | 是 | 合成音频位数 | 16 |
| tts.channels | int | 是 | 合成音频通道数 | 1 |
| tts.encoding | string | 是 | 合成音频格式说明如下  **raw**：pcm音频 **lame**： mp3音频 **opus**：8k opus音频 **opus-wb**：16k opus音频 **speex**：8k speex音频 **speex-wb**：16k opus音频  音频压缩关系可查看[**【附录】**](#附录) 了解 | raw |
| tts.sample\_rate | int | 是 | 合成音频采样率 ：8000、16000  注意与合成音频压缩格式对应 | 16000 |
...
包含 event事件结果（下发前端点、后端点事件）、iat（识别）结果、nlp（大模型）结果、tts（合成）结果等。
...
识别结果在 payload.iat.text 字段中（注意示例为流式结果，当前默认识别无流式结果）
...
> 合成音频数据在 payload.tts.audio 中，需要客户端 base64 decode，可按照音频参数encoding、sample\_rate、bit\_depth 播放

---
title: 参数配置说明
source_url: https://aiui-doc.xf-yun.com/project-1/doc-13/
---
...
[**- AIUI静态基础参数设置>>>点击跳转**](#AIUI静态基础参数设置)
  [**- AIUI动态参数设置概述>>>点击跳转**](#AIUI动态参数设置概述)
...
| interact\_mode | 是 | **交互模式设置** continuous：持续交互，对于语音即“一次唤醒，多次交互” oneshot：（默认模式）一次交互，对于语音即“一次唤醒，一次交互” |
...
| **vad** | 音频端点检测参数 | vad\_enable | 是 | **VAD启用设置** 1：(默认参数)开启vad 0：关闭vad |
...
| vad\_bos | 否 | **VAD前端超时时间** 单位：毫秒 取值范围：[1000,10000] 默认值： "5000" |
| vad\_eos | 否 | **VAD后端超时时间** 单位：毫秒 取值范围：[0,10000] 默认值："1000" |
...
| speech\_timeout | 否 | **对话超时时间** vad\_bos开始说话时计时，超过设置的时间自动结束对话并回调vad\_eos，单位：毫秒，示例 "500"（默认值60000） |
...
| **iat** | 识别音频参数 | sample\_rate | 否 | **采样率** 16000 |
| **tts** | 合成和播放控制参数 | engine\_type | 否 | **主动合成引擎模型** cloud：云端合成，默认取值 local：本地合成 |

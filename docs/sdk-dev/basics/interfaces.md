...
`eventType`定义参照[AIUIEvent](/sdk-dev/basics/events)
...
`msgType`详见[AIUIMessage](/sdk-dev/basics/events)。
...
cfg内容参见[AIUI配置文件](/sdk-dev/basics/params)。
...
不同`IAIUIEvent`有不同`eventType`。不同`eventType`其余字段有不同的定义，详见[AIUIEvent](/sdk-dev/basics/events)
...
不同`IAIUIMessage`有不同的`msgType`。不同`msgType`时，其他参数有不同定义，详见[AIUIMessage](/sdk-dev/basics/events)

...
cfg 内容参见 [AIUI 配置文件](/sdk-dev/basics/params/)。
...
不同 `IAIUIEvent` 有不同 `eventType`，详见 [AIUIEvent](/sdk-dev/basics/events)。
...
不同 `IAIUIMessage` 有不同 `msgType`，详见 [AIUIMessage](/sdk-dev/basics/events)。

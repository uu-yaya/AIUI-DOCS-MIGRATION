...
| CMD\_TTS | 27 | 有 | **进行语音合成**    arg1表示控制语音合成命令  params包含合成参数   使用参考[云端TTS](/sdk-dev/features/tts)。 |
...
| EVENT\_VAD | 6 | **VAD事件**    arg1取值:0(VAD（端点检测） 开始说话)、1(音量)、2(VAD（端点检测） 结束说话)、3（没说话超时）。  当arg1取值为1时，arg2为音量大小。 |
...
| EVENT\_TTS | 15 | **语音合成事件**    合成状态以及合成进度，使用参考[云端TTS](/sdk-dev/features/tts)。 |

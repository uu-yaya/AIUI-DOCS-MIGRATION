...
1、注意传统语义链路下 **NLP（语义理解）** 与大模型相关链路下 **cbm\_semantic** 和 **NLP（语义理解）** 结果的区别.

2、合成 **TTS（语音合成）** 不区分在线和离线.
...
| esr\_fsa | 离线命令词结果 |
| TTS（语音合成） | 合成结果 |
| **在线交互-传统语义交互链路** | IAT（语音识别） | 语音识别结果 |
| NLP（语义理解） | 传统语义技能结果 |
| tpp | 应用后处理结果 |
| TTS（语音合成） | 合成结果 |
| itrans | 翻译结果 |
| **在线交互-通用大模型交互链路** | IAT（语音识别） | 语音识别结果 |
| cbm\_tidy | 语义规整结果 |
...
注意合成**TTS（语音合成）**结果解析后是音频数据流，不能和其他结果混合处理（json转义会直接报错）

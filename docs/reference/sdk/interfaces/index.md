---
title: SDK 接口说明
description: AIUI SDK 各平台接口定义、调用方式及返回值说明
---
...
::: info 说明
iOS / Windows / Linux 中命名与 Android 中类似，稍有不同，一般加前缀 `I` 表示接口。例如 `AIUIMessage` 在 Android 中为 `AIUIMessage`，在 iOS 中为 `IFlyAIUIMessage`，在 Windows / Linux 中为 `IAIUIMessage`。
:::

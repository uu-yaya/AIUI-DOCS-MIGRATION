---
title: 极速超拟人个性化 API
description: 用户个性化数据的上传、下载和删除接口，适用于大模型交互和极速超拟人交互链路
---
...
| 参数名 | 类型 | 必填 | 说明 | 示例 |
|---|---|---|---|---|
| app_id | string | 是 | 应用 AppID，从平台控制台创建的应用中获取 | `123456` |
| uid | string | 否 | 用户 ID，用于区分不同用户，最大长度 32 | `1234567890` |
...
| id_value | string | 是 | 当前维度对应的值（自定义级别不超过 32 位） | `1234567890` |
...
以 `res_name` 为 `IFLYTEK.telephone_contact` 为例，`payload.text.text` Base64 解码后格式如下（每行一条记录）：

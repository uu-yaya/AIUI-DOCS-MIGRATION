---
title: 极速超拟人链路鉴权
description: AIUI 极速超拟人链路 API 的 HMAC-SHA256 签名鉴权机制
---
...
| signature | string | 是 | 由 header 和 APISecret 进行 HMAC-SHA256 加密后 Base64 编码 | 参见下方生成步骤 |
...
| request-line | string | 是 | 由 `{method} {url.path} HTTP/1.1` 组成 | `GET /v3/aiint/sos HTTP/1.1` |

#### WebSocket 请求示例

请求地址为 `wss://aiui.xf-yun.com/v3/aiint/sos` 时：
...
请求地址为 `https://aiui.xf-yun.com/api`，请求类型为 DELETE 时：

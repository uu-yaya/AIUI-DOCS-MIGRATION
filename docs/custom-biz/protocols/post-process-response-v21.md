...
| card | 用于向设备的关联APP推送消息。支持三种卡片类型：Simple、Standard、List。 - 点击查看详情 | Object | 否 |
...
| request | response格式 |
| --- | --- |
| [LaunchRequest](/custom-biz/protocols/post-process-request) | 标准响应中的所有内容的均可根据业务需求选择包含 |
| [TextRequest](/custom-biz/protocols/post-process-request) | 标准响应中的所有内容的均可根据业务需求选择包含 |
| [IntentRequest](/custom-biz/protocols/post-process-request) | 标准响应中的所有内容的均可根据业务需求选择包含 |
| [SessionEndedRequest](/custom-biz/protocols/post-process-request) | - 若`reason`为`USER_INITIATED`，可以回复。回复内容中只能包括`outputSpeech`，且`ShouldEndSession`取值必须为`true`。  - 若`reason`为`ERROR`，不可回复。 |
